...
// create header(secret key encoded hash string)
...
func (api *APIClient) GetRealTimeTicker(symbol string, ch chan<- Ticker) {
	...
	log.Printf("connencting to %s", u.String())
...
func (api *APIClient) SendOrder(order *Order) (*ResponseSendChildOrder, error) {
	...
	url := "/v1/me/sendchildorder"
	resp, _ := api.doRequest("POST", url, map[string]string{}, data)
	var responce ResponseSendChildOrder
	if err := json.Unmarshal(resp, &responce); err != nil {
		return nil, err
	}
	return &responce, nil
}
...
func (api *APIClient) ListOrder(query map[string]string) ([]Order, error) {
	resp, err := api.doRequest("GET", "/1/me/getchildorders", query, nil)
	if err != nil {
		return nil, err
	}
	var responceListOrder []Order

	if err := json.Unmarshal(resp, &responceListOrder); err != nil {
		return nil, err
	}
	return responceListOrder, nil
